...
final class WorkforceCalendarRepository extends OracleRepository
    implements WorkforceCalendarRepositoryContract
{
    public function calendars(int $companyId): array
    {
        $statement = $this->connection()->prepare(
            'SELECT
                ...
                (
                    SELECT COUNT(*)
                    FROM workforce_holidays holidays
                    WHERE holidays.company_id =
                            calendars.company_id
                      AND holidays.calendar_id =
                            calendars.calendar_id
                      AND holidays.holiday_date >=
                            TRUNC(CURRENT_DATE)
                ) AS holiday_count
    ...
    public function saveDay(
        ...
        $statement = $this->connection()->prepare(
            'MERGE INTO workforce_calendar_days target
             USING (
                SELECT
                    :calendar_id AS calendar_id,
                    :iso_weekday AS iso_weekday,
                ...
                'SELECT calendars.*
                 ...
                 WHERE calendars.company_id =
                   ...
                   AND (
                        ...
                        OR calendars.is_default = 1
                   )
                 ORDER BY
                    CASE
                        WHEN schedules.schedule_id
                            IS NOT NULL
                        THEN 1
                        ELSE 0
                    END DESC,
