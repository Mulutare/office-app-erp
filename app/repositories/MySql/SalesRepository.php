...
final class SalesRepository extends MySqlRepository implements SalesRepositoryContract
{
    ...
    {
        return $this->catalogue(
            'SELECT p.product_id,p.sku,p.name,p.category,p.product_type,p.unit_of_measure,COALESCE((SELECT pc.proposed_price FROM sales_product_price_changes pc WHERE pc.company_id=p.company_id AND pc.product_id=p.product_id AND pc.status=\'approved\' AND pc.effective_from<=NOW() ORDER BY pc.effective_from DESC,pc.price_change_id DESC LIMIT 1),0) unit_price,COALESCE((SELECT pc.approved_discount_percent FROM sales_product_price_changes pc WHERE pc.company_id=p.company_id AND pc.product_id=p.product_id AND pc.status=\'approved\' AND pc.effective_from<=NOW() ORDER BY pc.effective_from DESC,pc.price_change_id DESC LIMIT 1),0) discount_percent,COALESCE((SELECT pc.approved_tax_percent FROM sales_product_price_changes pc WHERE pc.company_id=p.company_id AND pc.product_id=p.product_id AND pc.status=\'approved\' AND pc.effective_from<=NOW() ORDER BY pc.effective_from DESC,pc.price_change_id DESC LIMIT 1),0) tax_percent,p.commission_rate,p.serial_tracking,p.active,p.model_id,m.brand_id,m.product_family,m.mifi_subtype,m.model_name,b.name brand_name,m.active model_active,b.active brand_active
    ...
    {$s=$this->connection()->prepare('SELECT p.*,COALESCE(SUM(b.quantity_on_hand*b.average_unit_cost)/NULLIF(SUM(b.quantity_on_hand),0),0) weighted_average_cost,COALESCE(SUM(b.quantity_on_hand),0) quantity_on_hand FROM sales_products p LEFT JOIN inventory_stock_balances b ON b.company_id=p.company_id AND b.product_id=p.product_id LEFT JOIN inventory_warehouse_locations l ON l.company_id=b.company_id AND l.location_id=b.location_id AND l.location_usage=\'internal\' WHERE p.company_id=:company_id AND p.product_id=:id AND p.deleted_at IS NULL AND (b.stock_balance_id IS NULL OR l.location_id IS NOT NULL) GROUP BY p.product_id');$s->execute(['company_id'=>$companyId,'id'=>$productId]);$row=$s->fetch(PDO::FETCH_ASSOC);if(is_array($row)){$approved=$this->connection()->prepare("SELECT proposed_price,approved_discount_percent,approved_tax_percent FROM sales_product_price_changes WHERE company_id=? AND product_id=? AND status='approved' AND effective_from<=NOW() ORDER BY effective_from DESC,price_change_id DESC LIMIT 1");$approved->execute([$companyId,$productId]);$current=$approved->fetch(PDO::FETCH_ASSOC);$row['unit_price']=is_array($current)?$current['proposed_price']:0;$row['discount_percent']=is_array($current)?$current['approved_discount_percent']:0;$row['tax_percent']=is_array($current)?$current['approved_tax_percent']:0;}return is_array($row)?$row:null;}
    ...
    {
        $approved=$this->connection()->prepare("SELECT pc.price_change_id,pc.proposed_price unit_price,pc.approved_discount_per_unit discount_per_unit,pc.approved_discount_percent discount_percent,pc.approved_tax_percent tax_percent,pc.effective_from,pc.requested_at,u.display_name changed_by_name FROM sales_product_price_changes pc LEFT JOIN users u ON u.user_id=pc.requested_by WHERE pc.company_id=? AND pc.product_id=? AND pc.currency=? AND pc.status='approved' AND pc.effective_from<=? ORDER BY pc.effective_from DESC,pc.price_change_id DESC LIMIT 1");
        $approved->execute([$companyId,$productId,strtoupper($currency),strlen($date)===10?$date.' 23:59:59':$date]);
        return $approved->fetch(PDO::FETCH_ASSOC) ?: ['price_change_id'=>null,'unit_price'=>0.0,'discount_per_unit'=>0.0,'discount_percent'=>0.0,'tax_percent'=>0.0];
    }
