...
final class InventoryRepository extends MySqlRepository implements InventoryRepositoryContract
{
        ...
        try {
            ...
            foreach ($lines as $line) {
                ...
                $result = $this->completeStockMovement([
                    'companyId' => $companyId,
                    'productId' => $productId,
                    'sourceWarehouseId' => !empty($receipt['default_source_location_id'])
                        ? $warehouseId
                        : null,
                    'sourceLocationId' => !empty($receipt['default_source_location_id'])
                        ? (int) $receipt['default_source_location_id']
                        : null,
    ...
    {
        $connection = $this->connection();
        if (!$connection->inTransaction() || !is_finite($delta) || abs($delta) < 0.0005) {
            throw new RuntimeException('A valid Inventory reservation transaction is required.');
        }
        $balance = $this->stockBalanceForUpdate($companyId, $warehouseId, $locationId, $productId);
        if (!$balance || ($delta > 0 && (float) $balance['quantity_on_hand'] - (float) $balance['quantity_reserved'] + 0.0005 < $delta)
            || ($delta < 0 && (float) $balance['quantity_reserved'] + 0.0005 < -$delta)) {
            throw new RuntimeException('Available stock or the linked transfer reservation changed. Retry the original request.');
        }
        $statement = $connection->prepare('UPDATE inventory_stock_balances SET quantity_reserved=quantity_reserved+?,version_number=version_number+1 WHERE company_id=? AND stock_balance_id=?');
        $statement->execute([$delta, $companyId, (int) $balance['stock_balance_id']]);
    }
