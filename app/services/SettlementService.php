...
final class SettlementService
{
    ...
    public function listing(): array{return ['settlements'=>$this->repo->list($this->tenant->companyId()),'eligiblePayments'=>$this->repo->eligiblePayments($this->tenant->companyId()),'bankAccounts'=>$this->repo->bankAccounts($this->tenant->companyId())];}
    public function find(int $id): ?array{return $this->repo->find($this->tenant->companyId(),$id);}
    public function create(array $input,int $actor): array{try{$this->authorizeActor($actor,'sales.settlements.create');$ids=array_values(array_unique(array_filter(array_map('intval',is_array($input['payment_ids']??null)?$input['payment_ids']:[]))));if($actor<1||$ids===[]||(int)($input['bank_account_id']??0)<1)return $this->error('Select an active bank account and at least one eligible posted payment.');$id=$this->repo->create($this->tenant->companyId(),(int)$input['bank_account_id'],$ids,trim((string)($input['notes']??'')),$actor);$this->audit($actor,'settlement.created','sales_settlements',$id,['payment_ids'=>$ids]);return ['successful'=>true,'id'=>$id];}catch(\Throwable $e){return $this->error($e->getMessage());}}
    public function transition(int $id,string $action,string $reason,int $actor): array{try{$permission=match($action){'submit'=>'sales.settlements.submit','review'=>'sales.settlements.review','reconcile'=>'finance.settlements.reconcile','approve'=>'finance.settlements.approve',default=>throw new \RuntimeException('Unknown settlement action.')};$this->authorizeActor($actor,$permission);$this->repo->transition($this->tenant->companyId(),$id,$action,$reason,$actor);$this->audit($actor,'settlement.'.$action,'sales_settlements',$id,['reason'=>$reason]);return ['successful'=>true];}catch(\Throwable $e){return $this->error($e->getMessage());}}
    ...
    public function evidence(int $settlementId,int $confirmationId): ?array{return $this->repo->confirmation($this->tenant->companyId(),$settlementId,$confirmationId);}
