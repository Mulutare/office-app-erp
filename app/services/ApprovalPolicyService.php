...
final class ApprovalPolicyService
{
 public function assertAndRecord(int $company,string $action,string $subject,int $subjectId,float $amount,int $maker,int $checker,?string $reason=null):void
 {
  ...
  if(!(new ModuleRoleService())->permissionAllowed($company,$checker,(string)$policy['required_permission']))throw new RuntimeException('The checker lacks the approval level required by company policy.');
