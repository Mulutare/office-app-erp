...
final class FinanceAccountingWorkspaceService
{
    ...
    public function workspace(string $section, array $input): array
    {
        ...
        return match ($section) {
            ...
            'ledger' => ['title' => 'General Ledger', 'rows' => $this->ledger($company, $filters), 'columns' => ['posting_date' => 'Date', 'batch_number' => 'Journal', 'source_type' => 'Source', 'source_number' => 'Reference', 'account_code' => 'Account', 'account_name' => 'Name', 'description' => 'Description', 'debit_amount' => 'Debit', 'credit_amount' => 'Credit', 'currency' => 'Currency', 'posted_at' => 'Posted at']],
            ...
            'reports' => ['title' => 'Finance Reports', 'reports' => $this->reports($company, $filters)],
    ...
    private function invoices(int $company, string $type, array $filters): array
    {
        ...
        $extra = $payable ? ',i.supplier_invoice_number,o.po_number' : '';
        ...
        $sql = "SELECT $party,i.invoice_number,i.invoice_date,i.due_date,i.total_amount,ROUND(i.total_amount-i.residual_amount,2) paid_amount,i.residual_amount,i.currency,i.payment_status,CASE WHEN i.residual_amount<=0 THEN 'Settled' WHEN i.due_date>=CURRENT_DATE THEN 'Current' WHEN DATEDIFF(CURRENT_DATE,i.due_date)<=30 THEN '1-30' WHEN DATEDIFF(CURRENT_DATE,i.due_date)<=60 THEN '31-60' WHEN DATEDIFF(CURRENT_DATE,i.due_date)<=90 THEN '61-90' ELSE '90+' END aging_bucket $extra FROM finance_invoices i $join WHERE ".implode(' AND ', $where).' ORDER BY i.due_date ASC,i.invoice_id DESC LIMIT 500';
    ...
    private function reports(int $company, array $filters): array
    {
        $where = ["b.company_id=:company", "b.status='posted'"];
        $params = ['company' => $company];
        if ($filters['currency'] !== '') { $where[] = 'b.currency=:currency'; $params['currency'] = $filters['currency']; }
        ...
        $trial=$this->rows($sql.implode(' AND ',$where).$group,$params);
        ...
        $asOf=$this->rows($sql.implode(' AND ',$asOfWhere).$group,$asOfParams);
