...
/** Legacy role templates. Runtime entitlement uses explicit company grants. */
final class ModuleRoleService
{
    ...
    {
        return $this->permissionAllowed($company, $user, $module.'.module.enabled');
    }

    public function permissionAllowed(int $company, int $user, string $permission): bool
    {
        if ($company < 1 || $user < 1) return false;
        $membership = \db()->prepare('SELECT 1 FROM company_users cu JOIN users u ON u.user_id=cu.user_id WHERE cu.company_id=? AND cu.user_id=? AND cu.active=TRUE AND u.active=TRUE AND u.deleted_at IS NULL');
        $membership->execute([$company,$user]);
        if (!$membership->fetchColumn()) return false;
        $permissions = (new \App\Repositories\MySql\CompanyMembershipRepository())->permissionCodes($user,$company);
