...
final class ProcurementController
{
    ...
    public function bill(string $id):void{$this->mutate('procurement.bills.create',function()use($id){$this->service->assertOrderAccess((int)$id,$this->actor());return $this->service->createBill((int)$id,\postString('supplier_invoice_number'),$this->actor());},'/procurement/'.(int)$id);}
