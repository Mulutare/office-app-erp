...
| `app/controllers/HomeController.php` | `SELECT DATABASE()` | Returns the active MySQL schema | Adapter metadata or `SYS_CONTEXT('USERENV','CURRENT_SCHEMA')` | High | Database health/metadata interface | Health response for both drivers |
| `app/services/DashboardService.php` | `CURDATE()` and `+ INTERVAL 1 DAY` | Uses server-local date boundaries | `TRUNC(CURRENT_DATE)` and Oracle date arithmetic | High | Dialect date expressions or bound UTC ranges | Midnight and time-zone boundary tests |
| `app/models/Company.php` | `NOW()`, pagination and `lastInsertId()` | Server timestamps, `LIMIT/OFFSET` and generated IDs | `SYSTIMESTAMP`, `OFFSET/FETCH`, identity/sequence with returning | Critical | Company repository plus dialect pagination/generated-ID APIs | Create, approve, search and pagination |
| `app/models/CompanyMembership.php` | `NOW()`, `LIMIT 1`, `ON DUPLICATE KEY UPDATE` | Expiry checks, row limiting and MySQL upsert | Oracle timestamp, `FETCH FIRST`, `MERGE` | Critical | Membership repository with explicit upsert | Concurrent assignment, default membership and expiry |
| `app/models/CompanyModule.php` | `NOW()`, `LIMIT 1` | Current entitlement filtering | Oracle timestamp and row limiting | High | Module-entitlement repository | Active, trial, expired and disabled modules |
| `app/models/User.php` | `NOW()`, pagination, allowlisted ordering and `lastInsertId()` | Authentication, paging, updates and IDs | Oracle expressions, `OFFSET/FETCH`, identity/sequence return | Critical | `UserRepositoryInterface` and dialect APIs | Authentication, lockout, create, paging and sort |
| `app/models/Role.php` | `LIMIT`, `FOR UPDATE`, `ON DUPLICATE KEY UPDATE` | Row limiting, locking and permission upsert | `FETCH FIRST`, Oracle `FOR UPDATE`, `MERGE` | Critical | Role repository with lock and replace-permission methods | Concurrent role updates and rollback |
| `app/models/UserActivity.php` | `CONCAT`, `CAST(... AS CHAR)`, MySQL collations | Builds a unified textual timeline | `||`, `TO_CHAR`, Oracle collation strategy | Critical | Activity-query repository per engine | Unicode, identifiers, search and ordering |
| `app/models/Department.php` | `LIMIT 1`, `lastInsertId()` | Single-row lookup and generated ID | `FETCH FIRST`, identity/sequence return | High | Department repository | Duplicate, create, lookup and IDs |
| `app/models/Employee.php` | Pagination, fixed `LIMIT 250`, `lastInsertId()` | Employee search and generated IDs | `OFFSET/FETCH`, identity/sequence return | Critical | Employee repository and pagination object | Large filters, create and tenant scoping |
| `app/models/ExpenseRequest.php` | `LIMIT/OFFSET` | Paged finance requests | `OFFSET/FETCH` | High | Expense request repository | Stable ordering, filters and tenant scoping |
| `app/models/AuditLog.php` | `LIMIT`, JSON encoded into long text | Recent logs and text snapshots | `FETCH FIRST`, CLOB or supported Oracle JSON type | Critical | Audit repository and long-text binding abstraction | 100k rows, CLOB, Unicode and redaction |
| `app/models/AuditLogQuery.php` | Pagination and dynamic conditions | Paged audit search | `OFFSET/FETCH` | High | Audit query repository and portable filters | Search, stable paging and execution plan |
| `app/models/LoginAttempt.php` | `LIMIT` | Recent authentication attempts | `FETCH FIRST` | Medium | Login-attempt repository | Lockout history and high volume |
| `app/models/EmployeeActivity.php` | `LIMIT/OFFSET` | Paged activity timeline | `OFFSET/FETCH` | Medium | Employee activity repository | Stable chronological pagination |
