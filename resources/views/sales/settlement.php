...
$settlementBase = str_contains((string)($_SERVER['REQUEST_URI'] ?? ''), '/finance/') ? '/finance/settlements' : '/sales/settlements';
...
<div class="module-stack settlement-workspace">
    ...
    <section class="card erp-record-header">
        <div><a class="erp-back-link" href="<?= e(appBasePath() . $settlementBase) ?>">&larr; Settlement history</a><p class="erp-eyebrow">Sales settlement</p><h2><?= e($settlement['settlement_number']) ?></h2><p>Settlement evidence, bank reconciliation and approval history</p></div>
    ...
    <div class="erp-document-actions" aria-label="Settlement documents"><a class="btn btn-secondary btn-compact" href="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/deposit-advice.pdf') ?>">Download Deposit Advice PDF</a><?php if (in_array($workflow, ['finance_reconciled', 'approved', 'closed'], true)): ?><a class="btn btn-secondary btn-compact" href="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/reconciliation.pdf') ?>">Download Reconciliation PDF</a><?php endif; ?></div>
    ...
    <section class="card table-card erp-section-card"><header class="erp-section-header"><div><p class="erp-eyebrow">Deposit basis</p><h2>Included Sales Payments</h2></div><span><?= e(count($settlement['lines'])) ?> payment(s)</span></header><div class="table-responsive"><table class="data-table erp-data-table"><thead><tr><th>Order</th><th>Customer</th><th>Payment</th><th>Date</th><th>Reference</th><th>DSA / DSP Receipt</th><th class="erp-money-column">Amount</th></tr></thead><tbody><?php foreach ($settlement['lines'] as $line): ?><tr><td><strong><?= e($line['order_number']) ?></strong></td><td><?= e($line['customer_name']) ?></td><td><?= e($line['payment_number']) ?></td><td><?= e($line['payment_date']) ?></td><td><?= e($line['reference_number'] ?? '-') ?></td><td><?php if (!empty($line['quick_sale_id']) && !empty($line['quick_sale_report_id']) && !empty($line['quick_sale_has_receipt'])): ?><div><strong><?= e($line['quick_sale_receipt_reference'] ?? 'Receipt') ?></strong></div><a class="btn btn-secondary btn-compact" href="<?= e(appBasePath() . '/sales/quick-sale/' . $line['quick_sale_id'] . '/reports/' . $line['quick_sale_report_id'] . '/evidence') ?>" target="_blank" rel="noopener">View DSA Receipt</a><?php else: ?><span>-</span><?php endif; ?></td><td class="erp-money erp-money-column"><?= e($money($line['amount'])) ?></td></tr><?php endforeach; ?></tbody></table></div></section>
    ...
    <?php if ($hasWorkflowAction): ?><section class="card erp-action-bar"><div><p class="erp-eyebrow">Next step</p><h2>Workflow Action</h2></div><div class="erp-action-group">
        <?php if ($workflow === 'draft' && $can('sales.settlements.submit')): ?><form method="post" action="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/submit') ?>"><?= csrfField() ?><button class="btn btn-primary">Submit Settlement</button></form><?php endif; ?>
        <?php if ($workflow === 'submitted' && $canReview): ?><form method="post" action="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/review') ?>"><?= csrfField() ?><button class="btn btn-primary">Review Settlement</button></form><?php endif; ?>
        <?php if ($workflow === 'supervisor_reviewed' && in_array($reconciliation, ['matched', 'partial', 'mismatch', 'review_required'], true) && $can('finance.settlements.reconcile')): ?><form method="post" action="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/reconcile') ?>"><?= csrfField() ?><button class="btn btn-primary">Reconcile</button></form><?php endif; ?>
        <?php if ($workflow === 'finance_reconciled' && $can('finance.settlements.approve')): ?><form method="post" action="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/approve') ?>"><?= csrfField() ?><button class="btn btn-primary">Approve</button></form><?php endif; ?>
    </div></section><?php endif; ?>

    <?php if (in_array($workflow, ['submitted', 'supervisor_reviewed'], true) && (float) $settlement['remaining_amount'] > 0.005 && $reconciliation !== 'matched' && $can('finance.bank_confirmations.create')): ?><section class="card erp-section-card"><header class="erp-section-header"><div><p class="erp-eyebrow">Financial evidence</p><h2>Add Bank Confirmation</h2><p>Upload original bank or mobile-wallet evidence (PDF, JPEG or PNG).</p></div></header><form method="post" enctype="multipart/form-data" action="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/confirmations') ?>"><?= csrfField() ?><div class="erp-form-grid erp-form-grid-three"><label class="form-field">Bank Reference<input name="bank_reference" maxlength="190" autocomplete="off" required></label><label class="form-field">Transaction Date<input type="date" name="transaction_date" required></label><label class="form-field">Confirmed Amount<input type="number" name="confirmed_amount" min="0.01" step="0.01" inputmode="decimal" required></label><label class="form-field erp-field-wide">Original Bank Receipt<input type="file" name="evidence" accept="application/pdf,image/png,image/jpeg" required><small>Maximum size and file-type validation remain enforced by the server.</small></label></div><div class="erp-form-actions"><button class="btn btn-primary">Add Bank Confirmation</button></div></form></section><?php endif; ?>

    <section class="card table-card erp-section-card"><header class="erp-section-header"><div><p class="erp-eyebrow">Evidence register</p><h2>Bank Confirmations</h2></div><span><?= e(count($settlement['confirmations'])) ?> confirmation(s)</span></header><div class="table-responsive"><table class="data-table erp-data-table"><thead><tr><th>Reference</th><th>Transaction Date</th><th class="erp-money-column">Confirmed Amount</th><th>Evidence</th><th>Added By</th></tr></thead><tbody><?php if ($settlement['confirmations'] === []): ?><tr><td colspan="5" class="empty-state">No bank confirmation has been added.</td></tr><?php endif; ?><?php foreach ($settlement['confirmations'] as $confirmation): ?><tr><td><strong><?= e($confirmation['bank_reference']) ?></strong></td><td><?= e($confirmation['transaction_date']) ?></td><td class="erp-money erp-money-column"><?= e($money($confirmation['confirmed_amount'])) ?></td><td><a class="btn btn-secondary btn-compact" href="<?= e(appBasePath() . $settlementBase . '/' . $settlement['settlement_id'] . '/confirmations/' . $confirmation['confirmation_id'] . '/evidence') ?>">View Receipt</a></td><td><?= e($confirmation['creator_name']) ?></td></tr><?php endforeach; ?></tbody></table></div></section>
