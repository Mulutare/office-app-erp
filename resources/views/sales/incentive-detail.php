...
<section class="card">
    ...
    <div class="detail-grid">
        ...
        <div><strong>Current unexplained variance for this claim</strong><br><?=e($claim['unexplained_variance'].' '.$claim['currency'])?></div>
        <?php else: ?>
        ...
        <div><strong>Historical unexplained variance</strong><br><?=e($claim['unexplained_variance'].' '.$claim['currency'])?></div>
        <?php endif; ?>
        ...
        <div><strong>Approved incentive</strong><br><?=e(($claim['approved_amount']??'0.00').' '.$claim['currency'])?></div>
        <div><strong>Safaricom settled</strong><br><?=e($claim['settled_amount'].' '.$claim['currency'])?></div>
        <div><strong>Outstanding</strong><br><?=e($claim['outstanding'].' '.$claim['currency'])?></div>
    </div>
...
<?php if(in_array($claim['status'],['approved','partially_settled'],true)&&!empty($canSettleIncentive)):?><section class="card"><h3>Record Safaricom settlement</h3><form method="post" action="<?=e(appBasePath())?>/sales/incentives/<?=(int)$claim['incentive_claim_id']?>/settlements"><?=csrfField()?><div class="finance-filter-form"><label>Date<input type="date" name="settlement_date" value="<?=e(date('Y-m-d'))?>" required></label><label>Amount<input type="number" name="amount" min="0.01" max="<?=e($claim['outstanding'])?>" step="0.01" required></label><label>Currency<input name="currency" value="<?=e($claim['currency'])?>" readonly></label><label>Safaricom payment reference<input name="external_payment_reference" maxlength="190" required></label><label>Evidence/reference<input name="evidence_reference" maxlength="500"></label></div><button class="btn btn-primary">Record settlement</button></form></section><?php endif;?>
