...
$sectionLabels = [
    ...
    'teams' => 'DSA / DSP & Teams', 'deliveries' => 'Deliveries',
];
$actionTaskMap = [];
...
<section class="card table-card"><div class="table-summary"><strong>Shared product catalogue</strong><span><?=e(count($products))?> products</span></div><div class="table-responsive"><table class="data-table"><thead><tr><th>Product</th><th>Category</th><th>Semantics</th><th>UoM</th><th>Effective unit price</th><th>Discount / unit</th><th>Tax</th><th>Status</th></tr></thead><tbody><?php if($products===[]):?><tr><td colspan="8" class="empty-state">No products yet.</td></tr><?php endif;?><?php foreach($products as $product):?><tr><td><strong><a href="/office_app/public/sales/products/<?=e($product['product_id'])?>"><?=e($product['sku'].' - '.$product['name'])?></a></strong></td><td><?=e($product['category']??'Uncategorised')?></td><td><?=e(str_replace('_',' ',$product['product_type']))?></td><td><?=e($product['unit_of_measure'])?></td><td><?=(float)$product['unit_price']>0?e($money($product['unit_price'])):'Not priced'?></td><td><?=e(number_format((float)($product['discount_percent']??0),2))?>%</td><td><?=e(number_format((float)($product['tax_percent']??0),2))?>%</td><td><?=!empty($product['active'])?'Active':'Archived'?></td></tr><?php endforeach;?></tbody></table></div></section>
<?php endif; ?>
