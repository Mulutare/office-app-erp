...
<div class="sales-workspace" data-section="quotations">
    ...
    <?php else: ?>
        ...
        <?php if (!empty($data['canTransition']) && in_array($quotation['status'], ['draft', 'sent'], true)): ?><section class="card"><h2>Fulfillment source</h2><p>Select the exact operational source before creating the Sales Order.</p><details><summary>View exact available stock</summary><div class="table-responsive"><table class="data-table"><thead><tr><th>Warehouse</th><th>Source location</th><th>Product</th><th>Available</th></tr></thead><tbody><?php foreach(($data['fulfilmentAvailability']??[]) as $stock):?><tr><td><?=e($stock['warehouse_name'])?></td><td><?=e($stock['location_name'])?></td><td><?=e($stock['sku'].' - '.$stock['name'])?></td><td><?=e($stock['quantity_available'])?></td></tr><?php endforeach;?></tbody></table></div></details><div class="page-actions"><?php if ($quotation['status'] === 'draft'): ?><form method="post" action="/office_app/public/sales/quotations/<?= e($quotation['quotation_id']) ?>/send"><?= csrfField() ?><button class="btn btn-secondary">Mark sent</button></form><?php endif; ?><?php if (($quotation['effective_status'] ?? $quotation['status']) !== 'expired'): ?><form method="post" action="/office_app/public/sales/quotations/<?= e($quotation['quotation_id']) ?>/confirm"><?= csrfField() ?><label>Warehouse <select name="warehouse_id" required><option value="">Select warehouse</option><?php foreach(($data['fulfilmentWarehouses']??[]) as $w):?><option value="<?=e($w['warehouse_id'])?>"><?=e($w['code'].' - '.$w['name'])?></option><?php endforeach;?></select></label><label>Source Location <select name="source_location_id" required><option value="">Select source</option><?php foreach(($data['fulfilmentLocations']??[]) as $l):?><option value="<?=e($l['location_id'])?>" data-warehouse-id="<?=e($l['warehouse_id'])?>"><?=e($l['code'].' - '.$l['name'])?></option><?php endforeach;?></select></label><button class="btn btn-primary">Confirm quotation</button></form><?php endif; ?><form method="post" action="/office_app/public/sales/quotations/<?= e($quotation['quotation_id']) ?>/cancel"><?= csrfField() ?><button class="btn btn-secondary">Cancel quotation</button></form></div></section><?php endif; ?>
