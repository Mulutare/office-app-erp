...
$quickSaleEvidence = is_array($data['quickSaleEvidence'] ?? null)
    ? $data['quickSaleEvidence']
    : null;
...
<div class="module-stack">
    ...
    <nav class="card details-toolbar">
        <a class="btn btn-secondary" href="<?= e(appBasePath()) ?>/finance">Finance</a>
        <a class="btn btn-secondary" href="<?= e(appBasePath()) ?>/finance/customer-invoices">Customer Invoices</a>
        <?php if (!empty($invoice['sales_order_id'])): ?><a class="btn btn-secondary" href="<?= e(appBasePath()) ?>/sales/orders/<?= e((string) $invoice['sales_order_id']) ?>">Sales Order <?= e((string) $invoice['order_number']) ?></a><?php endif; ?>
        <a class="btn btn-secondary" href="<?= e(appBasePath()) ?>/finance/customer-invoices/<?= e((string) $invoice['invoice_id']) ?>/invoice.pdf">Download Invoice PDF</a>
    </nav>
    ...
    <?php if (is_array($quickSaleEvidence)): ?>
    <section class="card">
        <h2>DSA Sale Evidence</h2>

        <p>
            <strong>Quick Sale:</strong>
            <?= e((string) ($quickSaleEvidence['quotation_number'] ?? '')) ?>
            / ID <?= e((string) ($quickSaleEvidence['quick_sale_id'] ?? '')) ?>
            |
            <strong>Sales Order:</strong>
            <?= e((string) ($quickSaleEvidence['order_number'] ?? '')) ?>
            |
            <strong>Report:</strong>
            #<?= e((string) ($quickSaleEvidence['report_id'] ?? '')) ?>
        </p>

        <p>
            <strong>DSA / DSP:</strong>
            <?= e((string) ($quickSaleEvidence['agent_name'] ?? '')) ?>
            |
            <strong>Manager:</strong>
            <?= e((string) ($quickSaleEvidence['manager_name'] ?? '')) ?>
        </p>

        <p>
            <strong>DSA invoice / receipt no.:</strong>
            <?= e(
                trim((string) ($quickSaleEvidence['invoice_reference'] ?? '')) !== ''
                    ? (string) $quickSaleEvidence['invoice_reference']
                    : 'Not provided'
            ) ?>
            |
            <strong>DSA payment method:</strong>
            <?= e(
                trim((string) ($quickSaleEvidence['payment_method'] ?? '')) !== ''
                    ? ucwords(
                        str_replace(
                            '_',
                            ' ',
                            (string) $quickSaleEvidence['payment_method']
                        )
                    )
                    : 'Not provided'
            ) ?>
            |
            <strong>DSA payment reference:</strong>
            <?= e(
                trim((string) ($quickSaleEvidence['payment_reference'] ?? '')) !== ''
                    ? (string) $quickSaleEvidence['payment_reference']
                    : 'Not provided'
            ) ?>
        </p>

        <?php if (!empty($quickSaleEvidence['has_evidence'])): ?>
            <a
                class="btn btn-secondary"
                href="<?= e(appBasePath()) ?>/sales/quick-sale/<?= e(
                    (string) $quickSaleEvidence['quick_sale_id']
                ) ?>/reports/<?= e(
                    (string) $quickSaleEvidence['report_id']
                ) ?>/evidence"
                target="_blank"
                rel="noopener"
            >View DSA Receipt</a>
        ...
        <p class="page-description">
            DSA information is the submitted sale evidence.
            The attachment is supporting evidence only.
            Finance should record the actual accounting payment reference
            when one exists and should not invent one from the attachment.
        </p>
    </section>
    <?php endif; ?>
    ...
    <?php if ($canPost): ?><section class="card"><h2>Post Invoice</h2><p>Posting locks the commercial amounts and creates the receivable, revenue and tax journal entry.</p><form method="post" action="<?= e(appBasePath()) ?>/finance/customer-invoices/<?= e((string) $invoice['invoice_id']) ?>/post"><?= csrfField() ?><button class="btn btn-primary">Post Invoice</button></form></section><?php endif; ?>
    <?php if ($canPay): ?>
    <section class="card">
        <h2>Register Payment</h2>
        <form method="post" action="<?= e(appBasePath()) ?>/finance/customer-invoices/<?= e((string) $invoice['invoice_id']) ?>/payments">
            <?= csrfField() ?>
    ...
    <section class="card table-card" id="payments">
        ...
        <?php if ($payments !== []): ?><div class="details-toolbar"><?php foreach ($payments as $payment): ?><a class="btn btn-secondary" href="<?= e(appBasePath()) ?>/finance/payments/<?= e((string) $payment['payment_id']) ?>/receipt.pdf">Download Payment Receipt PDF - <?= e((string) $payment['payment_number']) ?></a><?php endforeach; ?></div><?php endif; ?>
    </section>
</div>
