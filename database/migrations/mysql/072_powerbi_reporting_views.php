...
SELECT CONCAT(m.movement_id,'-S') AS movement_leg_key,m.company_id,m.movement_id,
       ...
       CASE WHEN m.movement_type='fulfilment' THEN ROUND(m.completed_quantity*m.unit_cost,2) ELSE 0 END AS fulfilled_inventory_cost_value,
       ...
       0 AS sales_return_quantity,0 AS sales_return_inventory_cost_value,
...
SQL,
        <<<'SQL'
-- Commercial fulfilled revenue is intentionally unavailable in this view.
-- inventory_pickings links fulfilment to the Sales order, but the production schema
-- has no authoritative fulfilment-to-sales_order_line allocation. Because one product
-- can occur on multiple order lines, allocating selling value by product or proportion
-- would fabricate revenue. Inventory-cost valuation remains authoritative.
...
SELECT im.company_id,im.movement_leg_key,im.movement_id,im.flow_type,
       ...
       o.order_id,o.order_number,o.order_date,o.status AS order_status,o.currency AS order_currency,
       ...
       CASE WHEN im.flow_type='FULFILMENT_SOLD_OUT' THEN im.fulfilled_inventory_cost_value ELSE 0 END AS fulfilled_inventory_cost_value,
       CASE WHEN im.flow_type='SALES_RETURN_IN' THEN im.absolute_quantity ELSE 0 END AS returned_quantity,
       CASE WHEN im.flow_type='SALES_RETURN_IN' THEN im.sales_return_inventory_cost_value ELSE 0 END AS returned_inventory_cost_value,
       CASE WHEN im.flow_type='FULFILMENT_SOLD_OUT' THEN im.absolute_quantity ELSE -im.absolute_quantity END AS net_sold_quantity,
       CASE WHEN im.flow_type='FULFILMENT_SOLD_OUT' THEN im.fulfilled_inventory_cost_value ELSE -im.sales_return_inventory_cost_value END AS net_sold_inventory_cost_value,
       CAST(NULL AS DECIMAL(18,2)) AS fulfilled_sales_amount,
       CAST(NULL AS DECIMAL(18,2)) AS returned_sales_amount,
       CAST(NULL AS DECIMAL(18,2)) AS net_sales_amount,
       'NOT_AVAILABLE_NO_FULFILMENT_TO_ORDER_LINE_REVENUE_ALLOCATION' AS commercial_value_status,
...
SELECT d.company_id,d.reporting_date,d.warehouse_id,d.pbi_shop_id,d.location_id,
       ...
       d.other_inbound_quantity,d.other_inbound_value,d.sales_return_quantity,d.sales_return_inventory_cost_value,
       ...
       d.fulfilled_sold_quantity,d.fulfilled_inventory_cost_value,d.transfer_out_quantity,d.transfer_out_value,
       ...
       d.fulfilled_inventory_cost_value-d.sales_return_inventory_cost_value AS net_sold_inventory_cost_value,
...
FROM (
    SELECT company_id,transaction_date AS reporting_date,warehouse_id,pbi_shop_id,
           ...
           SUM(sales_return_quantity) AS sales_return_quantity,SUM(sales_return_inventory_cost_value) AS sales_return_inventory_cost_value,
           ...
           SUM(fulfilled_sold_quantity) AS fulfilled_sold_quantity,SUM(fulfilled_inventory_cost_value) AS fulfilled_inventory_cost_value,
