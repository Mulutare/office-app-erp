...
return static function(PDO $c):string{
    ...
    $steps=array_map('intval',$c->query("SELECT statement_number FROM schema_migration_steps WHERE version='049' ORDER BY statement_number")->fetchAll(PDO::FETCH_COLUMN));
    if($steps===[]){if($global===0&&$company===0)return'baseline';return'apply';}
    if($steps===[1]&&$company===0)return'apply';
    if($steps===[1,2]&&$company===0&&$global===0)return'apply';
    throw new RuntimeException('Migration 049 recovery state is inconsistent.');
};
